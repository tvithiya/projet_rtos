...
void Tache1(void* p) 
{
  while (1) 
  {
    ...
    xQueueSendToBack(Queue1, &val_analogique, portMAX_DELAY );
...
void Tache2(void* p) 
{
  while (1) 
  {
    ...
    val_numerique =  tmp1 + tmp2;
    xQueueSendToBack(Queue2, &val_numerique, portMAX_DELAY );
    vTaskDelay( 1000 / portTICK_PERIOD_MS ); // wait for one second
  }
}
...
void Tache3(void* p) 
{
  while (1) 
  {
    if ((xQueueReceive( Queue1, &vals.analogique, portMAX_DELAY ) == pdTRUE) && (xQueueReceive( Queue2, &vals.numerique, portMAX_DELAY ) == pdTRUE)) 
    {
      vals.t_ms = millis();
